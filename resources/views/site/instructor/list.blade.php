@extends('site.layout')
@php
use App\Models\Instructor;
@endphp
@section('content')
    <main class="main" style="background-color:#fff;">
        <section class="innerBanner">
            <div class="ibPic"><img src="{{ asset('assets/images/courses-banner.jpg') }}" /></div>
            <div class="overlay"></div>
            <div class="ibCont text-center txtwhite">
                <h1>Find A Professionals That Suits You !!</h1>
                <form method="get">
                    @csrf
                    <div class="input-group">
                        <div class="form-outline">
                            <input type="text" name="q" value="{{ $q }}" class="form-control"
                                placeholder="Search by Name">
                        </div>
                        <button type="submit" class="btn btn-green ms-2"> <i class="fa fa-search" aria-hidden="true"></i>
                            Search </button>
                    </div>
                </form>
            </div>
        </section>
        <section>
            <div class="container  mb-4">
                <div class="titleWithAllbtn pt-5">
                    <h1 class="txtBlue">{{ $title }}</h1>
                </div>
                <div class="row teacherCards gx-4 mt-2">
                  
                    @if ($instructors->count() > 0)
                        @foreach ($instructors as $instructor)
                            @php 
                              $total_students = 0;
                              $avg_rating = Instructor::avg_rating($instructor->id); 
                            @endphp
                            @if (count($instructor->courses) > 0)
                                @foreach ($instructor->courses as $course)
                                    @php  $total_students = $total_students + $course->sales->count(); @endphp
                                @endforeach
                            
                              <div class="col-lg-4 col-md-4 col-sm-12">
                                  <div class="card">
                                      <div class="d-flex align-items-center">
                                          <div class="image"><a href="{{ route($instructor->slug) }}">
                                                  @if ($instructor->image)
                                                      <img src="{{ asset('storage/uploads/instructor/' . $instructor->image) }}"
                                                          class="rounded" width="155">
                                                  @else
                                                      <img src="{{ asset('assets/images/teacher.png') }}" class="rounded"
                                                          width="155">
                                                  @endif
                                              </a></div>
                                          <div class="ml-3 w-100">
                                              <h3 class="mb-2 mt-0">{{ $instructor->name }}</h3>
                                              <h4>{{ $instructor->professional }}</h4>
                                              <div
                                                  class="p-2 mt-2 bg-primary d-flex justify-content-between rounded text-white stats">
                                                  <div class="d-flex flex-column"> <span class="articles">Courses</span> <span
                                                          class="number1">{{ count($instructor->courses) }}</span> </div>
                                                  <div class="d-flex flex-column"> <span class="followers">Students</span>
                                                      <span class="number2">{{ $total_students }}</span> </div>
                                                  <div class="d-flex flex-column"> <span class="rating">Rating</span> <span
                                                          class="number3">{{ $avg_rating }}</span> </div>
                                              </div>
                                              <div class="button mt-2 d-flex flex-row align-items-center">
                                                  <button type="button" class="btn btn-sm btn-green w-100"
                                                      onClick="window.location.href='{{ route($instructor->slug) }}'">View
                                                      Profile</button>
                                              </div>
                                          </div>
                                      </div>
                                  </div>
                              </div>
                            @endif
                        @endforeach
                        <div class="align-middle pt-5"> {{ $instructors->links('pagination::bootstrap-4') }} </div>
                    @else
                        <div class="align-middle pt-5">Sorry !! No data found !!!</div>
                    @endif
                </div>
            </div>
        </section>
        @include('site.includes.partner')
    </main>
@endsection
@section('footer-scripts')
@endsection
