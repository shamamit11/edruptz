  ...
  <div class="app-main"> @include('instructor.includes.nav')
    <div class="app-main__outer">
      <div class="app-main__inner">
          ...
          <div class="page-title-wrapper">
            ...
            <div class="page-title-actions">
              <div class="d-inline-block">
                <button type="button" class="btn-shadow btn btn-success" onclick="window.location.href='{{ route('instructor-lesson-add', ['course_id='. Crypt::encryptString($row_course->id)]) }}'"> <span class="btn-icon-wrapper pr-2 opacity-7"> <i class="fa fa-plus fa-w-20"></i> </span> Add Lesson </button>
              </div>
            </div>
          </div>
        </div>
              ...
              <div class="nestable">
                <ul class="nestable-list">
                  ...
                  <li class="nestable-item nestable-item-handle" id="lesson-{{ $lesson->id }}">
                    <div class="nestable-handle"><i class="fa fa-bars"></i></div>
                    <div class="nestable-content">
                      <div class="media align-items-center">
                        <div class="media-body">
                          <h5 class="card-title h6 mb-0"> {{ $lesson->name }}</h5>
                        </div>
                        <div class="media-right"> <a href="{{route('instructor-lesson-add', ['course_id='. Crypt::encryptString($lesson->course_id).'&id='. Crypt::encryptString($lesson->id)])}}" class="btn btn-white btn-sm"><i class="fa fa-pencil"> Edit</i></a> <a class="btn btn-white btn-sm delete-row-btn" data-id="{{ $lesson->id }}" href="javacript:;"><i class="fa fa-trash"> Delete</i></a></div>
                      </div>
                    </div>
                  </li>
                  @endforeach
        ...
        <nav>
          <div class="align-middle"> {{ $lessons->links('pagination::bootstrap-4') }} </div>
        </nav>
      </div>
      @else
