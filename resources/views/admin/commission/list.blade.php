    ...
    <div class="card mb-10">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <nav class="navbar navbar-light">
                ...
                <a href="{{ route('admin-commission-add') }}" class="btn btn-primary my-2 my-sm-0 ms-1"> Add</a>
            </nav>
        </div>
        <div class="card-body"> @include('admin.includes.alert')
            ...
            <table class="table table-striped">
                <thead>
                    <tr>
                        <th scope="col" width="63">#</th>
                        <th scope="col" width="282">Name</th>
                        <th width="155" scope="col">Commission</th>
                        <th scope="col" width="227" style="text-align:center">Default</th>
                        <th scope="col" width="221" style="text-align:center">Status</th>
                ...
                <tbody>
                    ...
                    <tr id="tr{{ $row->id }}">
                        ...
                        <td><label class="switch" style="margin: 0 auto">
                                <input class="switch-input switch-default" type="checkbox" data-id="{{ $row->id }}"
                                    data-default-value="{{ $row->default }}" @if($row->
                                default == 1) checked @endif /> <span class="switch-label" data-on="Yes"
                                    data-off="No"></span> <span class="switch-handle"></span> </label></td>
                        <td><label class="switch" style="margin: 0 auto">
                        ...
                        <td style="text-align:center"><a href="{{route('admin-commission-add', ['id='.$row->id])}}"
...
$(document).ready(function() {
    $('.switch-default').change(function() {
        if ($(this).attr('data-default-value') == 0) {
            var val = 1;
        } else {
            var val = 0;
        }
    ...
    $('.delete-row-btn').click(function() {
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin-commission-delete")}}',
                    type: 'POST',
                    data: {
                        'id': id,
                        '_token': '{{ csrf_token() }}'
