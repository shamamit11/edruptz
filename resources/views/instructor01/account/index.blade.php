              ...
              <div class="card">
                ...
                <div class="card-body">
                  <div class="tab">
                    ...
                    <?php /*?><button class="tablinks" onclick="openCity(event, 'billing')">Billing</button>
                    <button class="tablinks" onclick="openCity(event, '')">Delete account</button><?php */?>
                  </div>
                </div>
              </div>
            </div>
            <div class="col-lg-8 col-md-8 col-sm-12"> @include('instructor.includes.alert')
                      ...
                      <div class="col-md-8 order-2 order-md-1">
                        <form action="{{ $profile_action }}" method="post" enctype="multipart/form-data">
                          ...
                          <div class="position-relative form-group">
                            ...
                            <input  name="profession" type="text" value="{{old('profession' , isset($user->professional)? $user->professional : '' )}}" class="form-control">
                            @if($errors->has('profession'))
                            <div class="error">{{$errors->first('profession')}}</div>
                            @endif </div>
                          <div class="position-relative form-group">
                            <label>Location</label>
                            <input  name="location" type="text" value="{{old('location' , isset($user->address)? $user->address : '' )}}" class="form-control">
                          ...
                          <div class="position-relative form-group">
                            ...
                            <textarea name="about_me" class="form-control description">{{old('about_me' , isset($user->about_me)? $user->about_me : '' )}}</textarea>
                          </div>
                          <div class="position-relative form-group">
                      ...
                      <div class="col-md-4 order-1 order-md-2 text-center">
                        <div class="avatar-holder mb-3">
                          <div class="proPic picCricle mx-auto"> @if($user && $user->image)
                            <div id="image">
                              <div style="margin:5px 0 0 0;"> <img src="{{asset('/storage/uploads/instructor/'.$user->image)}}"
    ...
    confirmDelete = function(field_name) {
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("instructor-account-imagedelete")}}',
                    type: 'POST',
                    data: {
                        'id': '{{ @$user->id }}',
